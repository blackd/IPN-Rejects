...
import org.anti_ad.mc.ipnrejects.buildsrc.fabricCommonAfterEvaluate
import org.anti_ad.mc.ipnrejects.buildsrc.fabricCommonDependency
import org.anti_ad.mc.ipnrejects.buildsrc.fabricRegisterCommonTasks
...
import org.anti_ad.mc.ipnrejects.buildsrc.Loaders.*
...
loom {
    ...
    runConfigs["client"].programArgs.addAll(listOf<String>("--width=1280",
                                                           "--height=720",
                                                           "--username=DEV"))
    mixin.defaultRefmapName.set("ipnrejects-refmap.json")
    accessWidenerPath.set {
        project.layout.projectDirectory.file("src/main/resources/ipn-rejects.accesswidener").asFile
    }
}
...
val remapped = tasks.named<RemapJarTask>("remapJar") {
    group = "fabric"
    dependsOn(proguard)
    val outFile = proguard.get().outputs.files.first()
    this.inputFile.set(outFile)
    archiveFileName.set(outFile.name.replace("-all-proguard", "-remapped"))
