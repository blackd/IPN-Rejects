...
import org.anti_ad.mc.ipnrejects.buildsrc.forgeCommonDependency
    ...
    runs {
        val runConfig = Action<RunConfig> {
            properties(mapOf(
                ...
                "bsl.debug" to "true"))
            arg("-mixin.config=mixins.ipn-rejects.json")
            arg("-mixin.config=mixins.ipn-rejects.cheats.json")
...
configure<CurseExtension> {
    ...
    project(closureOf<CurseProject> {
        ...
        releaseType = "release"
        supported_minecraft_versions[Loaders.CURSEFORGE]!!.forEach  {
...
modrinth {
    ...
    gameVersions.addAll(supported_minecraft_versions[Loaders.MODRINTH]!!)
    logger.lifecycle("""
        +*************************************************+
        Will release ${remappedJarFile.path}
        +*************************************************+
